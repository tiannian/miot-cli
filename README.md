...
# 查看设备信息、局域网 IP 和 token（如缓存中存在）及 MIoT 能力
...
`miot device get <did>` 会显示设备基本信息；缓存中存在 `ip`、`localip`、`local_ip` 或 `lan_ip` 时，会额外显示 IP 列；存在非空 `token` 时也会显示 token 列。随后它会依据设备型号查询公开 MIoT Spec v2 服务，并以表格列出该设备支持的属性、事件和操作。命令需要联网，但不需要额外的小米账号权限。
